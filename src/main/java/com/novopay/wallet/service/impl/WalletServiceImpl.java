...
public class WalletServiceImpl implements WalletService {
    ...
    public ResponseDto addMoneyInWallet(String walletId, String amount, String paymentType) {
        ...
        if (paymentService != null) {
            ...
            if (paymentResponse.getResultCode().equals(ApplicationConstants.SUCCESS)) {
                ...
                Transaction transaction = Transaction.builder().destination(wallet.getId()).source(paymentResponse.getTransactionId()).paymentType(PaymentType.CARD_PAYMENT).wallet(wallet).amount(balance).build();
    ...
    public ResponseDto sendMoneyInWallet(String sourceWalletId, String destMobileNumber, String amount) {
        ...
        Wallet destWallet = walletRepository.findByUser_MobileNumber(destMobileNumber);
        if (destWallet == null) {
        ...
        Transaction transaction = Transaction.builder().destination(destWallet.getId()).source(wallet.getId()).paymentType(PaymentType.WALLET).wallet(wallet).amount(balance).build();
